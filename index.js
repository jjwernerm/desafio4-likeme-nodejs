...
const { consultarPosts, crearPost, modificarPost, eliminarPost } = require('./consultaSQL');
...
// Creo una ruta POST con Express que reciba y almacene en PostgreSQL un nuevo registro + captura de error try catch
app.post("/posts", async (req, res) => {
  try {
    const { titulo, url, descripcion, likes } = req.body
    await crearPost(titulo, url, descripcion, likes)
    res.send("Post creado sin error")
  } catch (error) {
    const { titulo } = error
    if (titulo == 0) {
      res.status(400)
      .send("Error al crear el post")
    } else {
...
// Creo una ruta PUT con Express para modificar el título de un post en la tabla posts + captura de error try catch
app.put("/posts/:id", async (req, res) => {
  const { id } = req.params
  const { titulo } = req.query
  try {
    await modificarPost(titulo, id)
    res.send("El título del Post ha sido modificado sin error")
  } catch ({ code, message }) {
